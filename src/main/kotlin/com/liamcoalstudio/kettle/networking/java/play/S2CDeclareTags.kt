...
class S2CDeclareTags : Packet(0x5b, null) {
    ...
    fun addEmptyTag(buf: Buffer, string: String) {
        buf.addString(string)
        buf.addVarInt(0)
    }

    fun addEmptyTags(buf: Buffer, vararg strings: String) {
        buf.addVarInt(strings.size)
        strings.forEach { addEmptyTag(buf, it) }
    }
}